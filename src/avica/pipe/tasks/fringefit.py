...
from avica.pipe.core import PersistentMpiCasaRunner, FringeFit
from avica.util import c, save_metafile
from avica.ms.fringefit import find_refant_fromdf
...
def casatask_fringefit(
    ...
    """Returns CASA task fringefit command used by avica pipeline for single reference antenna.
...
def exec_FFT_fringefit(
    ...
    save_metafile(
        self.metafolder / "sources.avica", metad=self.dic_field
    )
    save_metafile(
        self.metafolder / "refants.avica",
